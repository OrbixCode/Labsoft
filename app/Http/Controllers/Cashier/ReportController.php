...
class ReportController extends Controller
{
    public function index(){
        ...
        $branchadd = Branch::where('id',$branch_id)->first();
        // dd($branchadd);
        return view('cashier.report')->with(compact('testsetup','genders','doctors','branchadd','testpara'));
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function($row){
                    ...
                    <a  class="btn btn-success btn-sm" href="'.route('bill.cprint',$row['id']).'" >Reciept</a> <button class="edit btn btn-secondary btn-sm " data-id="'.$row['id'].'"  id="TestView">View</button> <button class="delete btn btn-danger btn-sm" data-id="'.$row['id'].'"  id="DeleteTestReport">X</button>
                     ';
                    return $actionBtn;
                })
    ...
    public function bill($id){
        ...
        // dd($payment_recepit);
        return view('cashier.reciept')->with(compact('reports','testreportitem','payment_recepit','currentTime'));
    }
